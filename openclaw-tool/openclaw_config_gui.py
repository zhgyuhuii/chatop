...
PARAM_HINTS = {
    ...
    "gateway.bind": "绑定地址：loopback 仅本机（127.0.0.1）；all 监听所有网卡，需配合认证使用。",
    "gateway.auth.mode": "认证方式：token（推荐，本地也建议保留）、password、off（仅完全信任本机时使用）。",
...
def add_label_with_hint(parent, row, label_text, hint_key, column=0):
    ...
    lb = _gui_label(f, text=label_text)
...
class OpenClawConfigApp:
    ...
    def add_tab_gateway(self, f):
        ...
        _gui_combobox(f, self.vars["gateway.bind"], ["loopback", "all"], width_chars=14).grid(row=r, column=1, sticky=tk.W); r += 1
        ...
        _gui_combobox(f, self.vars["gateway.auth.mode"], ["token", "password", "off"], width_chars=14).grid(row=r, column=1, sticky=tk.W); r += 1
