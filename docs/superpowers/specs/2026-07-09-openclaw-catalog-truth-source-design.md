...
### 已由 spike 证实（2026-07-09，一次性容器 `docker run --rm chatop-ai:1.2.3`，未动生产）

- **`openclaw plugins install wecom` 可用**。openclaw 自行把通道 id 解析为
  `@wecom/wecom-openclaw-plugin@2026.5.7` 并装成功（`Installed plugin: wecom-openclaw-plugin`, rc=0）。
  → **安装不依赖 `npm_spec`**，dist JS 解析就此离开关键路径：解析失败只丢中文名/QR 标记，
  装插件不受影响。`npm_spec` 降级为纯展示字段。
- **装完后 schema 仍无字段**。`config schema` 的 `channels.properties` 从 25 键增至 26 键，
  但 `channels.wecom` **没有 `properties`**。→ **自由键值编辑器是必需路径，非可选**。
- **额外发现**：`openclaw-weixin` 与 `twitch` 虽在 schema 里，`properties` 同样为空。故
  `has_schema` 的判据必须是「有非空 properties」而非「键存在」，否则 GUI 会渲染出一个
  没有任何输入框的空表单。需要自由键值编辑器的通道共 5 个，不是 3 个。
- **`channels list --json` 的 `origin` 有第三个取值 `available`**（已装未配置）。

### 仍未证实

- **`models auth login --provider <id>` 是否校验 id**（TTY 检查先于 provider 校验短路）。故
  「`glm` 会让登录按钮失败」目前只是强推断。P1 仅修 id 本身（`glm`→`zai` 有独立证据：
  `zai` 有 14 个 GLM 模型，`glm` 前缀零模型）；登录按钮行为留 P3 在真 TTY 下验证。

### 其它

- **dist JS 文件名带内容哈希**，升级即变名，且 dist 里有**两份**（external-only 20 通道 /
  超集 26 通道）。做法：glob 后取**最大**的一份；解析失败则中文名回落到本地表。
- **刷新一次约 30s**（真机实测，比预估的 43s 快）。命令互不依赖、可并发，但本机仅 7.3G
  内存且生产容器常驻 → **默认串行**。
- **Amazon Bedrock / 智谱 GLM / 阿里云百炼「是假选项」的旧结论已被推翻**：Bedrock 是插件
  `@openclaw/amazon-bedrock-provider`，GLM 是内置 provider `zai`（14 个模型），百炼对应
  `@openclaw/qwen-provider`。三者 openclaw 都支持。provider 维度不是完整性问题，
  而是 id 正确性问题。
