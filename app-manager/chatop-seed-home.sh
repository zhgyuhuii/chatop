...
WANT=8   # 升级镜像、要补播新增工具时把这个数字 +1
         ...
         #     哨兵门控改为「自愈干净 且 磁盘充足」而非「cp -an 退出码为 0」——种子里 .launchpadlib
         #     等 root 私有目录会让 cp -an 恒返回非 0（权限不够，与磁盘无关），旧判据会导致哨兵
         #     永不落、每次开机空跑 6 万文件 cp（aidooo 生产实测暴露）。
...
# 自愈：卷内已是 0 字节、但种子里非空的文件——磁盘满时 cp 写残的残文件（cp -an 不覆盖，修不回）。
# 只修「卷内存在且为空、而种子非空」的：不碰用户改过的非空文件，也不碰种子本就为空的文件。
# 以「HOME 里的空文件」为遍历源（正常仅几百个，find -empty 亚秒级），反查种子——比扫种子全树
# （6 万+文件、纯 bash 循环 7s+）快 20 倍。结果写全局 heal_ok/heal_n。可重复调用（幂等）。
selfheal() {
  heal_ok=1; heal_n=0
  local dst rel src
...
# 磁盘是否充足（>512MB 可用）——用来区分「cp 因磁盘满失败(须重试)」与「cp 因良性原因(如 root 私有
# 目录权限)返回非 0(可放行)」。真磁盘满时截断的文件由 selfheal 兜住，这里只做哨兵门控的辅助判据。
ample_disk() {
  local kb; kb="$(df -Pk "$HOME" 2>/dev/null | awk 'NR==2{print $4}')"
  case "$kb" in ''|*[!0-9]*) return 0;; esac       # 取不到就当充足，别误锁
  [ "$kb" -gt 524288 ]
}

# 0) 自愈恒跑（不受版本哨兵约束）：修上一次开机/播种后遗留的截断文件。
selfheal
...
# 1) 补缺、绝不覆盖：用户数据/已装工具只补不改。全量 cp -an 还能补回被磁盘满整个漏建的文件
#    （自愈只修「存在但空」的，补不了「完全没有」的）——所以版本升级仍要跑一遍。cp -an 的退出码
#    不可靠（种子里 root 私有目录必致非 0），故忽略它，改用 cp 后再自愈一次 + 磁盘充足度判断。
cp -an "$SEED_SRC/." "$HOME/" 2>/dev/null || true
# 1b) cp 若因 ENOSPC 把新文件写残成 0 字节，此刻再自愈一遍兜住（cp 前的那次没见过这些新残文件）。
selfheal
[ "$heal_n" -gt 0 ] && echo "[seed] 播种后再自愈 $heal_n 个残文件"
...
if [ "$heal_ok" = 1 ] && ample_disk; then
  mkdir -p "$(dirname "$SENT")"
  echo "$WANT" > "$SENT"
  echo "[seed] done"
else
  echo "[seed] 播种未完成(磁盘空间不足或有残文件未补上)，不写版本哨兵，下次启动将重试" >&2
fi
